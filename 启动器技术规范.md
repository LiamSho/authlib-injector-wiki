...
- [启动游戏](#%E5%90%AF%E5%8A%A8%E6%B8%B8%E6%88%8F)
  ...
  - [添加启动参数](#%E6%B7%BB%E5%8A%A0%E5%90%AF%E5%8A%A8%E5%8F%82%E6%95%B0)
    ...
    - [添加 javaagent 参数](#%E6%B7%BB%E5%8A%A0-javaagent-%E5%8F%82%E6%95%B0)
    - [指定端类型](#%E6%8C%87%E5%AE%9A%E7%AB%AF%E7%B1%BB%E5%9E%8B)
...
- [地址协议补全](#%E5%9C%B0%E5%9D%80%E5%8D%8F%E8%AE%AE%E8%A1%A5%E5%85%A8)
...
### 添加 javaagent 参数
添加以下 JVM 参数，以加载 authlib-injector：
```
...
### 指定端类型
添加以下 JVM 参数，以告知 authlib-injector 其运行在 Minecraft 客户端上。
```
-Dauthlibinjector.side=client
```

### 配置预获取
启动器应在启动前请求 Yggdrasil API Root，并将响应内容（即 authlib-injector 的配置）通过以下 JVM 参数传入 authlib-injector：
```
-Dauthlibinjector.yggdrasil.prefetched={Base64 编码后的响应内容}
```

配置预获取将获取配置的任务交给了启动器，这使得配置获取操作可以和其他操作同时进行，进而减少启动耗时。除此之外，当用户的网络存在故障时，启动器在启动阶段便可以发现问题，并进行处理（报错或使用缓存的响应）。
